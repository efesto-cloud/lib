/**
 * Normalize any expand format into the standardized object format.
 *
 * Handles:
 * - undefined → {} (empty)
...
import type { Expand } from "./Expand.js";
import type { NormalizedExpand } from "./NormalizedExpand.js";
...
/**
 * Normalize a single expand spec or array of specs into the object format.
 *
 * @param spec - The expand specification(s) to normalize
 * @param shape - The shape definition (used to expand '*' and true)
 * @returns The normalized object representation
 */
export function normalizeExpand<T extends Record<string, unknown>>(
    spec: Expand<T> | Expand<T>[] | undefined,
    shape: T,
): NormalizedExpand<T> {
    if (spec === undefined) {
        return {} as NormalizedExpand<T>;
    }
    ...
    if (typeof spec === "string") {
        return { [spec]: true } as NormalizedExpand<T>;
    }
...
function expandAllFields<T extends Record<string, unknown>>(
    ...
    return result as NormalizedExpand<T>;
}
