...
MongoDB implementation of [`IUnitOfWork`](../unit-of-work). Wraps a `MongoClient` and exposes a `ClientSession` that repositories can use for session-scoped reads/writes inside `runWithTransaction`.

## Installation

```bash
pnpm add @efesto-cloud/mongodb-unit-of-work @efesto-cloud/unit-of-work mongodb
```
...
- Repositories should always thread `uow.session` into their MongoDB calls — forgetting to do so opts that operation out of the transaction.
