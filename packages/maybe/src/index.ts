...
export class Some<T extends NonNullable<unknown>> implements ISome<T> {
  ...
  flatMap<R extends NonNullable<unknown>>(fn: (value: T) => Maybe<R>): Maybe<R> {
    return fn(this.data);
  }
...
export namespace Maybe {
  export function maybe<T extends NonNullable<unknown>>(t?: T | null): Maybe<T> {
  ...
  export function fromObject<T extends NonNullable<unknown>>(obj: IMaybe<T>): Maybe<T> {
    return obj.some ? new Some(obj.data) : new None();
  }

  export function combine<T1 extends NonNullable<unknown>, T2 extends NonNullable<unknown>>(
    m1: Maybe<T1>,
    m2: Maybe<T2>
  ): Maybe<[T1, T2]> {
    return m1.isSome() && m2.isSome() ? new Some<[T1, T2]>([m1.data, m2.data]) : new None();
  }
}
