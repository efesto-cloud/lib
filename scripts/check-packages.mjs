...
for (const file of manifests) {
    ...
    if (!pkg.name?.startsWith('@efesto-cloud/')) {
        problems.push(`name must start with "@efesto-cloud/" (got "${pkg.name}")`)
    }

    const deps = { ...pkg.dependencies, ...pkg.peerDependencies, ...pkg.devDependencies }
    for (const [dep, range] of Object.entries(deps)) {
        if (typeof range === 'string' && range.startsWith('workspace:') && !allNames.has(dep)) {
            problems.push(`workspace dep "${dep}" does not resolve to a workspace package`)
        }
    }
    ...
    if (pkg.license !== 'MIT') problems.push(`license must be "MIT" (got "${pkg.license}")`)
